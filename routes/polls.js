...
router.get('/polls', async function(req, res) {
  ...
  try {
    ...
    const chainScript = blockchainVoting && !isGuest
      ? '<script>' +
        'function chainVote(pid,opt,optText){' +
          ...
          'window.hive_keychain.requestCustomJson("' + escape(name||'') + '","qr-cafe-vote","Posting","[]",json,"QR Cafe Vote",function(r){' +
            'if(r.success){window.location.href="/poll-vote?pid="+pid+"&opt="+opt;}' +
    ...
    res.send(page('Voting',
      '<h1>Voting</h1>' + pollHtml + chainScript +
...
router.get('/poll-vote', limitVote, async function(req, res) {
  ...
  try {
    ...
    if (!poll || !user || poll.status !== 'active') return res.redirect('/polls');
